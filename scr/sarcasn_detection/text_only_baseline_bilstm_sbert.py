...
# If you want to run the model for the sarcastic class only -> perceived vs. intended set --class_=sarcastic
## s_bert
# python text_only_baseline_bilstm_sbert.py --model=sbert --num_epochs=10 --loss_type=cross_entropy --tweet_sarc_csv=sarc_thread.csv --tweet_non_sarc_csv=/non_sarc_thread.csv --results_dir=baseline --class_=all
...
if __name__ == '__main__':
    ...
    if model_name.lower() != 'sbert':
        logging.info('Model {} not supported. Sbert only supported.'.format(model_name))
    ...
    if class_.lower() == 'sarcastic':
        ...
        dataset = AuthorTweetDataset(author_vocab=None, sarc_csv=sarc_csv, non_sarc_csv=non_sarc_csv, dim=2, punctuations=True)
    ...
    if class_.lower() == 'sarcastic':
        # no author sample and no author encoder, initialize for train
        initialize_raw_data_tweet(raw_dataset, dataset, train_tweets, 'train', process=True)
        initialize_raw_data_tweet(raw_dataset, dataset, val_tweets, 'val', process=True) # initialize for validation
        initialize_raw_data_tweet(raw_dataset, dataset, test_tweets, 'test', process=True)  # initialize for test

    else:
        initialize_raw_data(raw_dataset, dataset, train_tweets, 'train', None, None, process=True) # initialize for train
        initialize_raw_data(raw_dataset, dataset, val_tweets, 'val', None, None, process=True) # initialize for validation
        initialize_raw_data(raw_dataset, dataset, test_tweets, 'test', None, None, process=True) # initialize for test
    ...
    test_metrics = evaluate_metric(dataloader=test_dataloader, model=model, embedder='none', USE_AUTHORS=False, dataset=dataset, author_encoder=None,return_predictions=True)
    ...
    result_logs['model_name'] = args.model
